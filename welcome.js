...
module.exports = function (client) {
    ...
    client.on("guildMemberAdd", async member => {
      ...
      if (textString3.length >= 14) {
        ...
        ctx.fillStyle = '#FF613F';
        ctx.fillText(textString3, 850, canvas.height / 2 + 13);
      }
      //else dont do it
      else {
      ...
      ctx.fillText(textString2, 870, canvas.height / 2 + 50);
      //define the Member count
      var textString4 = `Member #${member.guild.memberCount}`;
      ctx.font = 'bold 60px "Roboto"';
      ctx.fillStyle = '#FDF6F4';
      ctx.fillText(textString4, 880, canvas.height / 2 + 125);
      //get the Guild Name
      var textString4 = `${member.guild.name}`;
      ctx.font = 'bold 60px "Roboto"';
      ctx.fillStyle = '#FDF6F4';
      ctx.fillText(textString4, 850, canvas.height / 2 - 150);
      //create a circular "mask"
      ctx.beginPath();
      ctx.arc(463, canvas.height / 2, 250, 0, Math.PI * 2, true);//position of img
      ctx.closePath();
      ctx.clip();
      //define the user avatar
      const avatar = await Canvas.loadImage(member.user.displayAvatarURL({ format: 'jpg' }));
      //draw the avatar
      ctx.drawImage(avatar, 213, canvas.height / 2 - 250, 500, 500);
      ...
      Hi <@${member.id}>! Please Read the Rules & Server Info, Have Fun and Enjoy!`)
